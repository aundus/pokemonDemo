...
function getRandomPokemon(){
  const url =  `/pokemon?id=${Math.floor(Math.random() * 100)}`
  window.location = url
}

(function($) {
  // Set the information to display a pokemon on the page
  function setPokemonInfo(pokemon){
    $("#name").text(defaultPokemon.name)
    $("#height").text(defaultPokemon.height)
    $("#weight").text(defaultPokemon.weight)
    $("#abilities").text(defaultPokemon.abilities.map((a => a.ability?.name)).join(', ') )
    $("#moves").text(defaultPokemon.moves.map((a => a.move?.name)).join(', '))
    $("#image").attr("src", defaultPokemon.sprites?.front_default)
  }


  // Return a pokemon by name or id
